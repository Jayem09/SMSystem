...
// StockTransfer represents a request to move inventory between branches.
type StockTransfer struct {
	...
	ApprovedByUserID    *uint               `json:"approved_by_user_id"` // Nullable, as it might be pending
	ReceivedByUserID    *uint               `json:"received_by_user_id"` // Nullable
...
// StockTransferItem represents the specific products and quantities requested in a transfer.
type StockTransferItem struct {
	ID              uint    `gorm:"primaryKey" json:"id"`
	StockTransferID uint    `gorm:"not null;index" json:"stock_transfer_id"`
	ProductID       uint    `gorm:"not null" json:"product_id"`
	Quantity        int     `gorm:"not null;check:quantity > 0" json:"quantity"`
	ReceivedQuantity int    `gorm:"not null;default:0" json:"received_quantity"` // Allowed partial receiving in future
